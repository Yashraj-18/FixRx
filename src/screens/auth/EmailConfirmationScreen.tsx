...
const EmailConfirmationScreen: React.FC = () => {
  ...
  const navigateAfterVerification = (resolvedUserType: UserType | null) => {
    ...
    console.log('🔄 Navigating after verification, userType:', resolvedUserType, 'userProfile:', userProfile);
    ...
    // ALWAYS go to UserType if no userType OR profile incomplete
    if (!resolvedUserType || !isProfileComplete) {
      console.log('👤 Navigating to UserType (no userType or incomplete profile)');
      resetTo('UserType');
      return;
    }

    // Only go to MainTabs if BOTH userType exists AND profile is complete
  ...
  useEffect(() => {
    // Check if deep link handler already handled magic link verification
    if (deepLinkHandler.hasHandledMagicLink) {
      console.log('✅ Deep link already handled magic link, skipping polling');
      hasNavigatedRef.current = true;
      return;
    }

    // Check if already authenticated (token exists) - deep link might have already handled it
    const checkInitialAuth = async () => {
      const isAlreadyAuth = await authService.isAuthenticated();
      if (isAlreadyAuth || deepLinkHandler.hasHandledMagicLink) {
        console.log('✅ Already authenticated on mount, skipping polling (deep link handled it)');
        hasNavigatedRef.current = true; // Prevent any navigation attempts
        return;
      }

      // Only poll for verification status if not already authenticated
      const interval = setInterval(() => {
        // Stop polling if deep link handled it or we've already navigated
        if (hasNavigatedRef.current || deepLinkHandler.hasHandledMagicLink) {
    ...
    const cleanup = checkInitialAuth();
    return () => {

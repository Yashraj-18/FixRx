  ...
  const loadAnalytics = async () => {
    try {
      ...
      if (userType === 'consumer') {
        promises.push(
          apiClient.get(`/analytics/user?timeRange=${selectedTimeRange}`),
          apiClient.get(`/analytics/business?timeRange=${selectedTimeRange}`)
        );
      } else {
        promises.push(
          apiClient.get(`/analytics/vendor?timeRange=${selectedTimeRange}`),
          apiClient.get(`/analytics/performance?timeRange=${selectedTimeRange}`)
        );
      }
      ...
      setAnalyticsData({
        userAnalytics: userType === 'consumer' ? results[0]?.data : null,
        vendorAnalytics: userType === 'vendor' ? results[0]?.data : null,
        performanceMetrics: results[1]?.data,
        businessMetrics: userType === 'consumer' ? results[1]?.data : null,
      });

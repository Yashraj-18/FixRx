...
const VendorProfileSetupScreen: React.FC = () => {
  ...
  const { userProfile, setUserProfile, userEmail, userPhone } = useAppContext();
  ...
  // Helper function to check if email is real (not a fake placeholder)
  const isRealEmail = (email: string) => email && !email.endsWith('@fixrx.app');
  ...
  const [formData, setFormData] = useState({
    ...
    phone: '',
    email: (() => {
      const emailCandidate = params?.email || userProfile?.email || userEmail || '';
      return isRealEmail(emailCandidate) ? emailCandidate : '';
    })(),
    businessName: '',
    professionalLicense: '',
    serviceArea: '',
  ...
  // Use ref to track if initial load is done to prevent double reload
  const initialLoadDone = useRef(false);

  React.useEffect(() => {
    // Skip if this is the very first render (data already set in useState)
    if (!initialLoadDone.current) {
      initialLoadDone.current = true;
      return;
    }
    
    const emailFromContext = params?.email || userProfile?.email || userEmail || '';
    const phoneFromContext = userProfile?.phone || userPhone || '';
    
    setFormData(prev => {
      let updated = { ...prev };
      let hasChanges = false;
      
      // Only update if email is real and different to avoid infinite loop
      if (isRealEmail(emailFromContext) && prev.email !== emailFromContext) {
        updated.email = emailFromContext;
        hasChanges = true;
      }
      
      // Only update if phone is different to avoid infinite loop
      if (phoneFromContext) {
        // Process phone number to remove country code
        const digitsOnly = phoneFromContext.replace(/\D/g, '');
        const phoneWithoutCountryCode = digitsOnly.length > 10 ? digitsOnly.slice(-10) : digitsOnly;
        
        if (prev.phone !== phoneWithoutCountryCode) {
          updated.phone = phoneWithoutCountryCode;
          hasChanges = true;
        }
      }
  ...
  }, [userEmail, userPhone, userProfile?.email, userProfile?.phone, params?.email]);
  ...
  const handleInputChange = (field: string, value: string) => {
    ...
    if (field === 'phone') {
      ...
      if (digitsOnly && validationErrors.includes(field)) {
    ...
    // Clear validation errors when field is filled
    if (value.trim() && validationErrors.includes(field)) {
      setValidationErrors(prev => prev.filter(error => error !== field));
    }
  };
  ...
  const validateCurrentStep = () => {
    if (currentStep === 1) {
      ...
      if (!formData.serviceArea) errors.push('Service Area');
      ...
      // Clear validation errors since all required fields are filled
      setValidationErrors([]);
      
      // Phone validation - check for formatted phone (123) 456-7890 or raw digits
      const phoneDigits = formData.phone.replace(/\D/g, '');
      if (phoneDigits.length !== 10) {
        console.log('❌ Phone validation failed:', formData.phone, 'digits:', phoneDigits.length);
        Alert.alert(
  ...
  const isStepOneComplete = useMemo(
    ...
    [formData.firstName, formData.lastName, formData.phone, formData.serviceArea]
  );

  const canContinue = useMemo(() => {
    if (currentStep === 1) {
      return isStepOneComplete && formData.phone.replace(/\D/g, '').length === 10;
    }
    return true; // Allow continuing from other steps
  }, [currentStep, isStepOneComplete, formData.phone]);
  ...
  const handleContinue = () => {
    ...
    if (currentStep === 1) {
      handleSubmit(); // This will handle the navigation after saving
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Use phone number as-is without country code prefix
      const phoneNumber = formData.phone ? formData.phone.replace(/\D/g, '') : '';

      // Update user profile in context with only the properties defined in UserProfile interface
      setUserProfile({
        ...userProfile,
        ...
        metroArea: formData.serviceArea,
        // Only include properties that exist in the UserProfile interface
        // professionalLicense, serviceRadius, yearsExperience, hasLiabilityInsurance, isBonded
        // will be stored in a separate vendor profile object or database
      });

      // For step 1, navigate to the service selection screen
      if (currentStep === 1) {
        navigation.navigate('VendorServiceSelection');
      } 
      // For other steps, update the current step
      else if (currentStep < 3) {
        setCurrentStep(prev => prev + 1);
      } else {
        // If this is the last step, navigate to main tabs
        navigation.navigate('MainTabs' as never);
      }
    } catch (error) {
                ...
                <View style={styles.inputContainer}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(text) => {
                      ...
                      const digitsOnly = text.replace(/\D/g, '').substring(0, 10);
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    // Clear validation error for service area
                    if (validationErrors.includes('Service Area')) {
                      setValidationErrors(prev => prev.filter(error => error !== 'Service Area'));
                    }
                  }}
                  activeOpacity={0.7}
                >

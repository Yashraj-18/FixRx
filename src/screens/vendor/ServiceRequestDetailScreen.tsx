...
type ServiceRequestDetailNavigationProp = StackNavigationProp<RootStackParamList, 'ServiceRequestDetail'>;

const ServiceRequestDetailScreen: React.FC = () => {
  const navigation = useNavigation<ServiceRequestDetailNavigationProp>();
  ...
  // Use photos from request data, fallback to service-specific placeholders
  const getServicePhotos = () => {
    if (request.photos && request.photos.length > 0) {
      return request.photos;
    }
    
    // Fallback photos based on service type
    const title = (request.title || request.service || '').toLowerCase();
    ...
    } else if (title.includes('kitchen') || title.includes('sink') || title.includes('garbage disposal')) {
      return [
    ...
    } else if (title.includes('bathroom') || title.includes('faucet') || title.includes('shower') || title.includes('toilet') || title.includes('bathtub')) {
      return [
    ...
    } else if (title.includes('pipe') || title.includes('leak') || title.includes('drain')) {
      return [
        'https://images.unsplash.com/photo-1607472586893-edb57bdc0e39?w=400&h=300&fit=crop&auto=format',
        'https://images.unsplash.com/photo-1585704032915-c3400ca199e7?w=400&h=300&fit=crop&auto=format'
      ];
  ...
  const requestPhotos = getServicePhotos();
  ...
  const handleMessage = () => {
    navigation.navigate('Messaging', {
      conversationId: 'mock-conversation-1', // Use consistent mock conversation ID
  ...
  const handleCall = () => {
    const phoneNumber = request.phone || getMockPhoneNumber(request.customerName);
    makePhoneCall(phoneNumber, request.customerName);
  };
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          text: 'Accept & Chat', 
          onPress: () => {
            // Navigate to messaging after acceptance
            navigation.navigate('Messaging', { 
              conversationId: 'mock-conversation-1', // Use consistent mock conversation ID
              customerName: request.customerName,
              userName: request.customerName,
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.footer, { backgroundColor: colors.card, borderTopColor: colors.border }]}>
        <TouchableOpacity style={[styles.declineButton, { backgroundColor: colors.card, borderColor: colors.border }]} onPress={handleDecline} activeOpacity={0.7}>

...
class ContactService {
  ...
  static async updateContact(contactId: string, contactData: Partial<Contact>): Promise<ApiResponse<Contact>> {
    try {
      const response = await apiClient.put<Contact>(`/contacts/${contactId}`, contactData);
  ...
  static async deleteContact(contactId: string): Promise<ApiResponse<{ success: boolean; deletedId: string }>> {
    try {
      const response = await apiClient.delete<{ success: boolean; deletedId: string }>(`/contacts/${contactId}`);
   ...
   */
  static async bulkCreateContacts(contacts: Omit<Contact, 'id' | 'createdAt' | 'updatedAt'>[], batchName?: string): Promise<ApiResponse<{
    successful: number;
    failed: number;
    duplicates: number;
    details: any;
  ...
  static async exportContacts(filters: ContactFilters = {}): Promise<string> {
    try {
      const response = await apiClient.get<string>('/contacts/export', {
        params: filters,
        responseType: 'text'
      });
